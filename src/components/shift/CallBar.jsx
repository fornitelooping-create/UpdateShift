...
import { Phone, PhoneOff, Mic, MicOff, PhoneIncoming, PhoneCall } from "lucide-react";
import UserAvatar from "./UserAvatar";
import { applyOutputDevice } from "@/lib/audioDeviceSettings";

export default function CallBar({ call }) {
  const { callState, remoteUserId, muted, connectionError, remoteAudioRef, remoteStream, acceptCall, declineCall, endCall, toggleMute } = call;
  ...
  return (
    <>
      ...
      {connectionError && callState === "idle" && (
        <div className="fixed bottom-16 left-1/2 -translate-x-1/2 z-[70] bg-[#ed4245] text-white text-sm px-4 py-2 rounded-lg shadow-xl">
          {connectionError}
        </div>
      )}
